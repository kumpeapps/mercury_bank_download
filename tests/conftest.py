...
TEST_DATABASE_URL = os.environ.get(
    "TEST_DATABASE_URL", "sqlite:///:memory:"  # Default to in-memory SQLite for tests
)
...
def register_test_routes(app, test_session):
    ...
    def register():
        ...
        try:
            ...
            if user_count == 1:
                ...
                super_admin_role = (
                    test_session.query(Role).filter_by(name="super-admin").first()
                )
    ...
    def admin_users():
        ...
        if not (
            ...
            and (current_user.has_role("admin") or current_user.has_role("super-admin"))
        ):
            return redirect(url_for("dashboard"))
    ...
    def edit_user_settings(user_id):
        ...
        if not (
            hasattr(current_user, "has_role") and current_user.has_role("super-admin")
        ):
            return redirect(url_for("dashboard"))
...
def init_roles(test_db):
    ...
    roles = [
        ...
        Role(
            name="transactions",
            description="Full access to transaction data",
            is_system_role=True,
        ),
