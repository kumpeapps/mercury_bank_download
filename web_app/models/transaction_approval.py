...
class TransactionRestriction(Base):
    ...
    def applies_to_transaction(self, account_id, amount, category=None, subcategory=None, transaction_date=None):
        ...
        if transaction_date and transaction_date < self.start_date:
            return False
...
class TransactionApproval(Base):
    ...
    def can_approve_transaction(self, amount, category=None, subcategory=None, merchant=None, transaction_date=None):
        ...
        try:
            # Handle timezone-aware/naive datetime comparison carefully
            start_date = self.approval_start_date
            end_date = self.approval_end_date
            
            # Make all datetimes timezone-naive for comparison
            if hasattr(check_date, 'tzinfo') and check_date.tzinfo:
            ...
            # Fallback: log error and return False for safety
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"Date comparison failed in can_approve_transaction: {str(e)}")
            return False
