#!/usr/bin/env python3
"""
Initial Database Setup for Mercury Bank Integration Platform - Web App
...
def create_initial_system_settings(engine):
    ...
    try:
        ...
        for setting_data in initial_settings:
            ...
            existing_setting = session.query(SystemSetting).filter(
                SystemSetting.key == setting_data['key']
            ).first()
            if not existing_setting:
                setting = SystemSetting(**setting_data)
                session.add(setting)
                logger.info(f"✅ Created system setting: {setting_data['key']} = {setting_data['value']}")
            else:
...
def main():
    """Main setup function"""
    logger.info("🚀 Starting Mercury Bank Web Application Initial Setup...")
    ...
    try:
        ...
        logger.info("✅ Mercury Bank Web Application initial setup completed successfully!")
