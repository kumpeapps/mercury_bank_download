                    ...
                    if (
                        ...
                        if user_account_access:
                            for account_id in user_account_access:
                                ...
                                if account and account.mercury_account_id == mercury_account_id:
                                    account.authorized_users.append(user)
                                    print(f"DEBUG: Granted user {user.username} access to account {account.id}")
                
                db_session.commit()
                flash("User access permissions have been updated successfully!", "success")
                return redirect(url_for("manage_mercury_access", mercury_account_id=mercury_account_id))

        # If just displaying the page (GET request)
        return render_template(
            "manage_mercury_access.html",
            mercury_account=mercury_account,
            accounts=all_accounts,
            all_users=all_users,
            title="Manage Mercury Account Access",
        )
    except Exception as e:
        db_session.rollback()
        flash(f"An error occurred: {str(e)}", "error")
        logger.error(f"Error in manage_mercury_access: {str(e)}")
        return redirect(url_for("accounts"))
    finally:
        db_session.close()
